...
import type { K8sEvent, K8sEventListParams, K8sEventType } from '../types/k8s-event';
...
/**
 * Build field selector string from filter options
 */
...
/**
 * Filter events client-side for multi-value filters
 * This handles cases where field selectors don't support multiple values
 */
function filterEventsClientSide(events: K8sEvent[], filters: EventsFeedFilters): K8sEvent[] {
  return events.filter((event) => {
    // Multi-value involved kinds filter
    if (filters.involvedKinds && filters.involvedKinds.length > 1) {
    ...
    // Multi-value source components filter
    if (filters.sourceComponents && filters.sourceComponents.length > 1) {
      if (!filters.sourceComponents.includes(event.source?.component || '')) {
        return false;
      }
    }

    // Multi-value namespaces filter
    if (filters.namespaces && filters.namespaces.length > 1) {
      if (!filters.namespaces.includes(event.metadata?.namespace || '')) {
        return false;
      }
    }
...
export function useEventsFeed({
  ...
  const buildParams = useCallback(
    (cursor?: string): K8sEventListParams => {
      return {
        namespace,
  ...
  const handleWatchEvent = useCallback((event: WatchEvent<K8sEvent>) => {
    ...
    // Check if this event matches the current filters
    ...
    if (event.type === 'ADDED') {
      ...
      setEvents((prev) => {
        // Check for duplicates by name and namespace
        const exists = prev.some(
          (e) =>
            e.metadata?.name === event.object.metadata?.name &&
            e.metadata?.namespace === event.object.metadata?.namespace
        );
        if (exists) {
          return prev;
        }
