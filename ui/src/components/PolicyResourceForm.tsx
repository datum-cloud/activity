...
export function PolicyResourceForm({
  ...
  const loadResources = useCallback(async () => {
    ...
    try {
      ...
      // Get the actual Kind from API discovery
      ...
      // Combine audited resources with their proper Kind from discovery
      const resourcesWithKind = auditedResources
        .filter((r) => r && resourceMap.has(r))
        .map((r) => ({
          name: r,
          kind: resourceMap.get(r) || resourceToKind(r), // Fallback to derived if not in discovery
        }));
