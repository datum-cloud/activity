...
// ActivityPolicyStorage implements a RESTStorage for ActivityPolicy.
type ActivityPolicyStorage struct {
	*genericregistry.Store
}

// ActivityPolicyStatusStorage implements the REST endpoint for updating ActivityPolicy status.
type ActivityPolicyStatusStorage struct {
...
func (s *ActivityPolicyStatusStorage) Destroy() {
	// No-op: the store is shared with the main REST storage
}

// Get retrieves the object from the storage.
func (s *ActivityPolicyStatusStorage) Get(ctx context.Context, name string, options *metav1.GetOptions) (runtime.Object, error) {
	return s.store.Get(ctx, name, options)
}

// Update alters the status subset of an object.
func (s *ActivityPolicyStatusStorage) Update(ctx context.Context, name string, objInfo rest.UpdatedObjectInfo, createValidation rest.ValidateObjectFunc, updateValidation rest.ValidateObjectUpdateFunc, forceAllowCreate bool, options *metav1.UpdateOptions) (runtime.Object, bool, error) {
...
func policyToTableRow(policy *v1alpha1.ActivityPolicy) metav1.TableRow {
	...
	if !policy.CreationTimestamp.IsZero() {
		age = duration.HumanDuration(metav1.Now().Sub(policy.CreationTimestamp.Time))
	}

	// Get Ready condition status
	ready := "Unknown"
	if cond := apimeta.FindStatusCondition(policy.Status.Conditions, "Ready"); cond != nil {
...
func NewStorage(scheme *runtime.Scheme, optsGetter generic.RESTOptionsGetter) (*ActivityPolicyStorage, *ActivityPolicyStatusStorage, error) {
	...
	return &ActivityPolicyStorage{store}, &ActivityPolicyStatusStorage{store: &statusStore}, nil
}
