...
func TestPolicyPreviewStorage_Create_SingleAuditMatch(t *testing.T) {
	...
	// Actor is alice@example.com, kind should be empty since objectRef doesn't have Kind
	expectedSummary := "alice@example.com created "
...
func TestPolicyPreviewStorage_Create_MultipleInputs(t *testing.T) {
	...
	// First activity
	if resultPreview.Status.Activities[0].Spec.Summary != "alice@example.com created " {
		t.Errorf("Unexpected first activity summary: %q", resultPreview.Status.Activities[0].Spec.Summary)
	}

	// Second activity
	if resultPreview.Status.Activities[1].Spec.Summary != "bob@example.com deleted " {
...
func TestPolicyPreviewStorage_Create_MixedAuditAndEvent(t *testing.T) {
	...
	if resultPreview.Status.Activities[1].Spec.Summary != "deploy-controller deployed HTTP Proxy" {
...
func TestPolicyPreviewStorage_Create_WithKindLabel(t *testing.T) {
	storage := NewPolicyPreviewStorage()
	...
	preview := &v1alpha1.PolicyPreview{
		Spec: v1alpha1.PolicyPreviewSpec{
			...
			KindLabel:       "Custom Proxy",
			KindLabelPlural: "Custom Proxies",
		},
	}
	...
	// Should use the custom kind label
	expectedSummary := "bob@example.com created Custom Proxy"
	if resultPreview.Status.Activities[0].Spec.Summary != expectedSummary {
		t.Errorf("Expected Summary=%q, got %q", expectedSummary, resultPreview.Status.Activities[0].Spec.Summary)
	}
}
...
func TestPolicyPreviewStorage_Create_ActivityFields(t *testing.T) {
	...
	// Check actor fields
	if activity.Spec.Actor.Type != "serviceaccount" {
		t.Errorf("Expected Actor.Type='serviceaccount', got %q", activity.Spec.Actor.Type)
	}
	if activity.Spec.Actor.Name != "my-sa" {
		t.Errorf("Expected Actor.Name='my-sa', got %q", activity.Spec.Actor.Name)
	}
