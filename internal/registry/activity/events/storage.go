...
import (
	...
	eventswatch "go.miloapis.com/activity/internal/watch"
)
...
type EventsREST struct {
	...
	watcher eventswatch.EventsWatcherInterface
}
...
func NewEventsRESTWithWatcher(backend EventsBackend, watcher eventswatch.EventsWatcherInterface) *EventsREST {
	return &EventsREST{
		backend: backend,
		watcher: watcher,
	}
...
func (r *EventsREST) Watch(ctx context.Context, options *metainternalversion.ListOptions) (watch.Interface, error) {
	if r.watcher == nil {
		return nil, errors.NewServiceUnavailable("Watch API is not available for events. Please contact your administrator.")
	}
	...
	// Build watch filter from options
	filter := eventswatch.EventWatchFilter{
		Namespace: namespace,
	}
	...
	if options.ResourceVersion != "" && options.ResourceVersion != "0" {
		...
		if err != nil {
			klog.V(4).InfoS("Invalid resourceVersion for events watch, starting from now",
	...
	// Extract field selector filters
	if options.FieldSelector != nil && !options.FieldSelector.Empty() {
		filter.FieldSelector = options.FieldSelector.String()

		if value, found := options.FieldSelector.RequiresExactMatch("involvedObject.kind"); found {
			filter.InvolvedObjectKind = value
		}
		if value, found := options.FieldSelector.RequiresExactMatch("involvedObject.name"); found {
			filter.InvolvedObjectName = value
		}
		if value, found := options.FieldSelector.RequiresExactMatch("reason"); found {
			filter.Reason = value
		}
		if value, found := options.FieldSelector.RequiresExactMatch("type"); found {
			filter.Type = value
		}
	}
