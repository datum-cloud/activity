...
type EventsNATSConfig struct {
	URL           string
	StreamName    string // JetStream stream name (e.g., "K8S_EVENTS")
	SubjectPrefix string // Subject prefix (e.g., "events.k8s")
	TLSEnabled    bool
	TLSCertFile   string
	TLSKeyFile    string
	TLSCAFile     string
...
func (w *EventWatch) matchesFilter(event *corev1.Event) bool {
	...
	if w.filter.FieldSelector != "" {
		...
		for _, term := range terms {
			value := w.getFieldValue(event, term.Column)
...
// getFieldValue extracts a field value from an event based on the column name.
func (w *EventWatch) getFieldValue(event *corev1.Event, column string) string {
	switch column {
	case "namespace":
