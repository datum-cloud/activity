...
// Global functions available in k6
...
const BASE_URL = __ENV.API_SERVER_URL || 'https://localhost:6443';
const API_PATH = '/apis/activity.miloapis.com/v1alpha1/auditlogqueries';
const TOKEN = __ENV.KUBE_TOKEN || '';
const NAMESPACE_FILTER = __ENV.NAMESPACE_FILTER || 'default';

// Client certificate paths (if using mTLS)
const CLIENT_CERT_PATH = __ENV.CLIENT_CERT_PATH || '';
const CLIENT_KEY_PATH = __ENV.CLIENT_KEY_PATH || '';

// Read certificate files during init phase (open() only works here)
const CLIENT_CERT_DATA = CLIENT_CERT_PATH ? open(CLIENT_CERT_PATH) : '';
const CLIENT_KEY_DATA = CLIENT_KEY_PATH ? open(CLIENT_KEY_PATH) : '';

// Build TLS auth configuration for k6 options
const tlsAuthConfig = CLIENT_CERT_DATA && CLIENT_KEY_DATA ? [{
  domains: [BASE_URL.replace(/^https?:\/\//, '').split(':')[0]],
  cert: CLIENT_CERT_DATA,
  key: CLIENT_KEY_DATA,
}] : undefined;

// Add TLS auth to options if configured
if (tlsAuthConfig) {
  options.tlsAuth = tlsAuthConfig;
}

// Query templates with different complexity levels
// Using correct API field names based on error messages:
...
const queryTemplates = [
  ...
  },
  {
    ...
    filter: `objectRef.namespace == '${NAMESPACE_FILTER}'`,
    limit: 100,
  },
  {
  ...
  // Medium complexity queries
  {
    ...
    filter: `verb == 'delete' && objectRef.namespace == '${NAMESPACE_FILTER}'`,
    limit: 100,
  },
  {
  ...
  // Complex queries
  {
    ...
    filter: `objectRef.namespace in ['${NAMESPACE_FILTER}', 'kube-system'] && objectRef.resource == 'deployments' && verb in ['create', 'update']`,
    limit: 100,
  },
  {
...
function getRequestOptions(): RequestOptions {
  ...
  // Note: TLS client certificates are configured globally in options.tlsAuth

  return options;
}

// Create AuditLogQuery request body
function createQueryRequest(template: typeof queryTemplates[0], continueAfter?: string): string {
...
function executeQuery(template: typeof queryTemplates[0]): void {
...
function executeQueryWithPagination(template: typeof queryTemplates[0], maxPages = 3): void {
  let continueAfter: string | undefined;
  let pageCount = 0;
...
export function setup(): void {
  ...
  console.log(`Using namespace filter: ${NAMESPACE_FILTER}`);
