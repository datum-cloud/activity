...
--
-- Using ReplacingMergeTree in a Replicated database ensures this tracking
-- table is automatically replicated across all replicas.
...
-- ==================================================================
-- ReplacingMergeTree provides:
-- - Deduplication based on ORDER BY key during merges
-- - Eventual consistency with quorum writes (configured via settings)
--
-- Replication is handled by the Replicated database engine (see database creation above).
-- When using a Replicated database, table engines should NOT include Keeper paths.
-- The database automatically:
-- - Replicates DDL changes across all replicas
-- - Coordinates metadata synchronization via Keeper
-- - Ensures data replication through the database-level coordination
--
