-- Migration: 007_activities_table_v2
-- Description: Recreate activities table with time-bucketed projections aligned with audit_logs patterns
...
-- Step 2: Create new table with updated schema and time-bucketed projections
...
(
    ...
    -- ========================================================================
    -- Projections (time-bucketed, aligned with audit_logs patterns)
    -- ========================================================================

    -- Projection for platform-wide queries by API group
    PROJECTION platform_query_projection
    (
        SELECT *
        ORDER BY (toStartOfHour(timestamp), timestamp, api_group, resource_kind, resource_uid)
    ),

    -- Projection for actor-name-based queries (platform-wide)
    PROJECTION actor_query_projection
    (
        SELECT *
        ORDER BY (toStartOfHour(timestamp), timestamp, actor_name, api_group, resource_kind, resource_uid)
    ),

    -- Projection for actor-uid-based queries (user scope)
    PROJECTION actor_uid_query_projection
    (
        SELECT *
        ORDER BY (toStartOfHour(timestamp), timestamp, actor_uid, api_group, resource_kind, resource_uid)
    )
)
ENGINE = ReplicatedReplacingMergeTree(reindex_version)
PARTITION BY toYYYYMMDD(timestamp)
-- Primary key uses toStartOfHour(timestamp) as the leading column for efficient
-- time-bucketed queries, matching the projection sort orders.
-- origin_id provides deduplication: the activity processor follows a
-- "first policy wins" rule so each source event produces at most one Activity.
ORDER BY (toStartOfHour(timestamp), timestamp, tenant_type, tenant_name, origin_id)
