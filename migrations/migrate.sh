...
CLICKHOUSE_TLS_CERT="${CLICKHOUSE_TLS_CERT:-}"
CLICKHOUSE_TLS_KEY="${CLICKHOUSE_TLS_KEY:-}"
CLICKHOUSE_TLS_CA="${CLICKHOUSE_TLS_CA:-}"
...
clickhouse_cmd() {
    local query="$1"
    local cmd="clickhouse-client --host=${CLICKHOUSE_HOST} --port=${CLICKHOUSE_PORT} --user=${CLICKHOUSE_USER}"

    if [ -n "${CLICKHOUSE_PASSWORD}" ]; then
        cmd="${cmd} --password=${CLICKHOUSE_PASSWORD}"
    fi

    # Add TLS/SSL options if secure connection is enabled
    if [ "${CLICKHOUSE_SECURE}" = "true" ]; then
        cmd="${cmd} --secure"

        if [ -n "${CLICKHOUSE_TLS_CERT}" ] && [ -f "${CLICKHOUSE_TLS_CERT}" ]; then
            cmd="${cmd} --openssl-cert=${CLICKHOUSE_TLS_CERT}"
        fi

        if [ -n "${CLICKHOUSE_TLS_KEY}" ] && [ -f "${CLICKHOUSE_TLS_KEY}" ]; then
            cmd="${cmd} --openssl-key=${CLICKHOUSE_TLS_KEY}"
        fi

        if [ -n "${CLICKHOUSE_TLS_CA}" ] && [ -f "${CLICKHOUSE_TLS_CA}" ]; then
            cmd="${cmd} --openssl-ca=${CLICKHOUSE_TLS_CA}"
        fi
    fi
...
apply_migration() {
    ...
    if cat "${migration_file}" | clickhouse-client \
